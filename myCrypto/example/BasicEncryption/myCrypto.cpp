...
String MyCrypto::encrypt(String plainText) {
  ...
  char encrypted[len];
  aesLib.encrypt64(plainText.c_str(), plainText.length(), (byte*)encrypted, aesKey, keySize * 8, aesIV);
...
String MyCrypto::decrypt(String encryptedText) {
  ...
  String cipher = encryptedText;//encryptedText.substring(16);
  int len = cipher.length() + 1;
  char decrypted[len];
  aesLib.decrypt64(cipher.c_str(), cipher.length(), (byte*)decrypted, aesKey, keySize * 8, aesIV);
